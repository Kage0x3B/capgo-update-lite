...
export async function sha256Hex(env: R2Env, key: string): Promise<string> {
    ...
    const digester = new (
        crypto as unknown as {
            DigestStream: new (algorithm: string) => { digest: Promise<ArrayBuffer> } & WritableStream<Uint8Array>;
        }
...
/**
 * Sign and send a HEAD request to the bucket. Used by /health to confirm the
 * endpoint is reachable and our credentials are still valid — any non-2xx
 * (including 403 from bad keys) counts as failure.
 */
export async function ping(env: R2Env): Promise<void> {
    const url = `${s3Endpoint(env)}/${BUCKET_NAME}`;
    const res = await client(env).fetch(url, { method: 'HEAD' });
    if (!res.ok) {
        throw new Error(`R2 HEAD ${BUCKET_NAME}: ${res.status}`);
    }
}
