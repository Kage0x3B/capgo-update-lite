...
async function zipDir(root: string): Promise<Uint8Array> {
    ...
    if (!hasIndex) {
        console.warn('[33mwarn:[0m no index.html at the root of the dist dir — the plugin may not boot the bundle');
    }
...
async function main() {
    ...
    console.log(`✓ published bundle ${init.bundle_id} (${version}) — state=${row.state} active=${row.active}`);
}
