...
<header class="mb-6">
    <h1 class="h3 sm:h2">Publishing from the CLI</h1>
    <p class="text-surface-600-400 text-sm">
        The browser upload flow above is convenient for one-off uploads; for everything else use
        <code>capgo-update-lite-cli</code>. The same admin token logged into this dashboard works as
        <code>CAPGO_ADMIN_TOKEN</code>.
    </p>
</header>

<section class="card preset-filled-surface-100-900 space-y-4 p-4 sm:p-5">
    <div>
...
export CAPGO_ADMIN_TOKEN="…"   # same value you logged in with</code
        ...
        <p class="text-surface-600-400 mt-2 text-xs">
            Anything that lives in env can also live in <code>capgo-update-lite.json</code> — the admin token is the one thing
            you should keep in env / secret store.
        </p>
    </div>
    ...
    <div>
        ...
        <p class="text-surface-600-400 mt-2 text-xs">
            ...
            <code>channel</code>, and <code>distDir</code> placeholders. Edit it once and commit it.
        </p>
    </div>
    ...
    <div>
        ...
        <pre class="bg-surface-900 text-surface-50 overflow-x-auto rounded p-3 text-xs"><code
            ...
            ></pre>
        <p class="text-surface-600-400 mt-2 text-xs">
            With <code>capgo-update.config.json</code> committed, that's the whole command. The bundle version is
            sourced from <code>package.json</code>; if it matches the active bundle on the channel, the CLI prompts for
            a <code>patch</code>/<code>minor</code>/<code>major</code>
            bump and writes the new value back to <code>package.json</code> before publishing. Native min-build floors
            are auto-detected from <code>android/</code> and
            <code>ios/</code>.
        </p>
    </div>
    ...
    <div>
        ...
        <pre class="bg-surface-900 text-surface-50 overflow-x-auto rounded p-3 text-xs"><code
            ...
            ></pre>
        <p class="text-surface-600-400 mt-2 text-xs">
            Activates an existing bundle without re-uploading — useful for rolling forward to a previously-deactivated
            version.
        </p>
    </div>
    ...
    <div>
        ...
        <p class="text-surface-600-400 text-xs">
            ...
            <code>--version-exists-ok</code> to make the publish idempotent (exits 0 if the version is already published).
        </p>
    </div>
</section>
